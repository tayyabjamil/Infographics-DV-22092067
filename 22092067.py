...
def show_selected_cities_map(ax, top_companies):
    ...
    merged_dataframe = pd.merge(top_companies_country_city, world_cities_data_with_lat_lng, left_on=[
                                'City', 'Country'], right_on=['city', 'country'], how='left')
    ...
    result_dataframe = pd.merge(top_companies, merged_dataframe, on=[
    ...
    world.boundary.plot(ax=ax, linewidth=0.8,
                        color='darkslategray', edgecolor='white')

    # Plot cities on the map with stylish marker styles
    ax.scatter(result_dataframe['lng'], result_dataframe['lat'], s=250, c=circle_color,
               marker='o', label='Top Unicorn Companies', edgecolors='black', linewidths=1, )

    ax.set_title('Geographical City distribution of Unicorn countries ',
    ...
    handles = [mpatches.Patch(
        color=circle_color, label=f'{country} ({count} companies)') for country, count in country_counts.items()]

    # Add legend outside the plot with increased font size
    legend = ax.legend(handles=handles, loc='lower center', bbox_to_anchor=(
        0.5, -0.7), fontsize=22, borderaxespad=0.1,  ncol=3)
...
def show_country_pie_chart(ax, unicorn_companies_data):
    ...
    explode_large = [
        0.1 if country in large_slices.index else 0 for country in country_counts.index]

    wedgeprops = {'linewidth': 3, 'edgecolor': 'grey'}  # Add a shadow effect

    # Set font sizes
    ax.set_title('Percentage of Top 50 Unicorn Companies in different Countries',
                 fontsize=30, fontweight='bold', color="red")
    ...
    legend_labels = [f'{country}: {percentage:.1f}%' for country, percentage in zip(
        country_counts.index, (country_counts / country_counts.sum()) * 100)]
    legend = ax.legend(legend_labels, title='Legend',
                       loc='center left', bbox_to_anchor=(-0.4, 0.5), fontsize=30)
...
def show_categories_vertical_bar_chart(ax, unicorn_companies_data):
    ...
    ax.set_title('Industry Distribution of Top Unicorn Companies Worldwide',
                 fontsize=30, fontweight='bold', color="red")
    # Rotate x-axis labels for better readability
    ax.set_xticklabels(ax.get_xticklabels(), rotation=45,
                       ha='right', fontsize=24)
    ax.set_xlabel('Industries', fontsize=30, fontweight='bold')
...
def line_chart(ax, unicorn_companies_data):
    ...
    unicorn_companies_data_subset['Valuation ($B)'] = unicorn_companies_data_subset['Valuation ($B)'].replace(
    ...
    ax.plot(valuations, companies, marker='o', linestyle='-',
            color='b', linewidth=2, markersize=8, label='Valuation')

    # Set chart title and labels
    ax.set_title('Top 30 Company Valuations in Billions',
                 color='red', fontsize=30)
    ax.set_ylabel('Companies',  fontsize=30, fontweight='bold')
...
# Plotting a scatter plot in the second grid (dummy data used)

# Plotting a packed bubble chart in the third grid
line_chart(axs[1, 0], unicorn_companies_data)

fig.text(0.75, 0.98, 'Name: Muhammad Tayyab, Reg: 22092067', fontsize=50, ha='center', va='center',
         color='white', fontweight='bold', bbox=dict(facecolor='lightblue', edgecolor='none', ))
fig.text(0.75, 0.95, 'Infographics of Top Unicorn Companies Analysis', fontsize=50, ha='center',
         va='center', color='white', fontweight='bold', bbox=dict(facecolor='lightblue', edgecolor='none', ))
